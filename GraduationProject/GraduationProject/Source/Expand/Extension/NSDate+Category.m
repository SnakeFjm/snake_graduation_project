...
@implementation CHMFormatterManager

+(NSDateFormatter*)shareCMFormatter
{
    static NSDateFormatter *sharedInstance = nil;
    
    static dispatch_once_t predicate; dispatch_once(&predicate, ^{
        sharedInstance = [[NSDateFormatter alloc] init];
        sharedInstance.locale = [NSLocale currentLocale];
       // sharedInstance.timeZone = [NSTimeZone systemTimeZone];
    });
 ...
 **按照指定的时间格式返回字符串
 */
-(NSString *)formattedTimeWidthFormat:(NSString*)format
{
    NSDateFormatter* formatter = [CHMFormatterManager shareCMFormatter];
...
{
    NSDateFormatter *dateFormatter = [CHMFormatterManager shareCMFormatter];
...
+ (NSDate *)dateWithYearsBeforeNow:(NSInteger)years {
    ...
    NSInteger todayYear = [today year];
    NSInteger beforeYear = todayYear - years;
    NSString *beforDateString = [NSString stringWithFormat:@"%4ld-%2ld-%2ld", (long)beforeYear, (long)today.month, (long)today.day];
    
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
    NSDate *beforeDate = [dateFormatter dateFromString:beforDateString];
